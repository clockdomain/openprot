...
//! I3C receiver process for the two-process interrupt-preemption test.
//!
//! Identical in behaviour to `tests/i3c_user_irq`: an `interrupt` object
//! (I3C = PIC IRQ 2) delivers the TTI RX-descriptor interrupt via a WaitGroup,
//! this process drains the frame through the [`I3cTarget`] facade, and
//! `interrupt_ack` re-enables the level-triggered source.
//!
//! The difference is what else is running. A sibling `spinner` process never
//! blocks, so while this process waits in `object_wait` the spinner is the
//! current thread. The I3C IRQ therefore wakes a thread that is *not* the one
//! it interrupted, which is the condition that forces the kernel through
//! `Arch::context_switch` from inside `trap_handler`. See `system.json5`.
...
use receiver_codegen::{handle, signals};
...
fn entry() {
    ...
    pw_log::info!("i3c irq preempt: waiting for private write");
                ...
                match frame {
                    Ok(Some(n)) => {
                        if n >= EXPECTED.len() && &buf[..EXPECTED.len()] == EXPECTED {
                            pw_log::info!("i3c irq preempt: received expected payload");
                            let _ = syscall::debug_shutdown(Ok(()));
                            loop {}
                        }
                        pw_log::error!("i3c irq preempt: payload mismatch len={}", n as u32);
